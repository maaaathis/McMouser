...
/**
 * Bridge to the native macOS keyboard fix (see src/main/native/macmc.mm).
 *
 ...
 *
 * <p>Only used on Minecraft 1.21+; on older versions this class is never
 * referenced (the keyboard mixins are excluded), so the dylib is never loaded.
 */
public final class Native {
    ...
    static {
        if (IS_MAC) {
            try {
                NativeUtils.loadLibraryFromJar("/natives/macmc.dylib");
            } catch (IOException e) {
                throw new RuntimeException("MacMc: failed to load native library", e);
            }
        }
    }
